...
const joinRandomGame = async (userID) => {
  try {
    ...
    while (!joinedGame) {
      // Keep trying to find an available game
...
const addUserToGame = async (gameID, userID) => {
  try {
    ...
    if (game.status !== Status.WAITING) {
      // Another user is already trying to join
...
const checkForWinner = async (gameID) => {
  try {
    ...
    for (let i = 0; i < 3; i++) {
      // Rows
      if (
        game.squares[3 * i] == game.squares[3 * i + 1] &&
        game.squares[3 * i] == game.squares[3 * i + 2]
      ) {
      ...
      // Columns
      if (
        game.squares[i] == game.squares[3 + i] &&
        game.squares[i] == game.squares[6 + i]
      ) {
    ...
    // Diagonals
    if (
      game.squares[0] == game.squares[4] &&
      game.squares[0] == game.squares[8]
    ) {
      winner = game.squares[0];
    } else if (
      game.squares[2] == game.squares[4] &&
      game.squares[2] == game.squares[6]
    ) {
...
const checkForTie = async (gameID) => {
  let game = await findGameByID(gameID);
  if (game.squares.filter((square) => !square).length == 0) {
