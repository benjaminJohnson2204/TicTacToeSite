...
io.of("/play").on("connection", (socket) => {
  socket.on("join", async (message) => {
    ...
    if (
      ...
      game.usernames.includes(message.username)
    ) {
      // Reconnecting
      io.of("/play")
        .to(message.id)
        .emit("opponent reconnected", { username: message.username });
    }
  ...
  socket.on("move", async (message) => {
    ...
    if (
      user._id.toString() !== game.turn ||
      !(await isSquareAvailable(message.gameID, message.row, message.col))
    ) {
      // Make sure it's this user's turn and the square is available
      io.of("/play").to(message.gameID).emit("game update", game);
      return;
    }
    let updatedGame = await insertSquare(
      message.gameID,
      user._id,
      message.row,
      message.col
